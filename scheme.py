...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        operands_eval = operands.map(curry)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        vals = scheme_eval(expr, env)
        env.define(target, vals)
        return target
        # END PROBLEM 6
        ...
        env.define(name, do_lambda_form(Pair(arg, rest),  env)) 
        return name
        # END PROBLEM 10
    else:
...
def do_or_form(expressions, env):
    ...
    if scheme_truep(val):
        return val
    if expressions.second == nil and scheme_falsep(val):
        return val
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        parameters,values = Pair(bindings.first.first, parameters), Pair(scheme_eval(bindings.first.second.first, env), values)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            def curry(operand):
                return scheme_eval(operand, env)
            rest_eval = rest.map(curry)
            result = scheme_apply(scheme_eval(first, env), rest_eval, env)
            # END PROBLEM 20
    return result
